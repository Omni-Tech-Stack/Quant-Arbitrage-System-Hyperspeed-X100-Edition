...
class MLAnalyticsEngine:
    ...
    def __init__(self, model_dir: str = "./models"):
        ...
        if DUAL_AI_AVAILABLE:
            try:
                ...
                print("[ML] ℹ Falling back to simple scoring")
    ...
    def retrain_model(self):
        ...
        Returns:
            bool: True if retraining was initiated, False otherwise
        """
        if self.dual_ai:
            # In production, would retrain with accumulated data
            print("[ML] ℹ Model retraining available with Dual AI engine")
            return True
        else:
            print("[ML] ℹ Model retraining requires Dual AI engine")
            return False
    ...
    def get_statistics(self):
        ...
        Returns:
            dict: Statistics about trades and model performance
        """
        stats = {
            'total_trades': 0,
            'engine_type': 'Dual AI' if self.dual_ai else 'Simple',
            'model_loaded': self.dual_ai is not None
        }
        
        if self.dual_ai and hasattr(self.dual_ai, 'trade_log_path'):
            # Count trades from log file if available
            try:
                import os
                if os.path.exists(self.dual_ai.trade_log_path):
                    with open(self.dual_ai.trade_log_path, 'r') as f:
                        stats['total_trades'] = sum(1 for _ in f)
            except:
                pass
